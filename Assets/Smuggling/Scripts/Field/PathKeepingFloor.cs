...
public class PathKeepingFloor : FloorData
{
    int pathKeeping;

    float dangerRate = 0.2f;
    List<Ground> unRideableGround = new List<Ground>();
    UnityEngine.AI.NavMeshPath floorPath;

    /*  予定
        １．まずランダムにBreakableとDangerだけを設置
        　　(このときDanger率を多めにするかは検討中、Disappearにすると後が面倒なのでしない)
        ２．Updateごとにこの階層をNavMeshだけで突破できるかどうか、パスを探す
        ３．パスがない場合
            ３－１．Dangerをランダムに選び、UnBrokenに変更して２に戻る
        ４．パスがあった場合
    ...
    */
    ...
    public override void Start()
    {
        ...
        for (int i = 1; i < groundData.Length; i++)
        {
            ...
            {
                groundData[i].init(GroundType.Breakable);
            }
        }
